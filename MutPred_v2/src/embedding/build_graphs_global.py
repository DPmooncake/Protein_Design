...
新增功能:
    - 支持图模式 graph_mode:
        * "pocket": 仅对口袋残基建蛋白节点 (原始行为);
        * "global": 对所有蛋白残基建节点, 但仍用 pocket_mask 标记 core/shell。
    - 支持指定 PDB 级 batch_size:
        * --batch_size 控制一次处理多少个 PDB 文件 (只影响外层循环和进度条刷新频率)。
...
输入:
    ...
    - --graph_mode:   "pocket" 或 "global", 决定蛋白节点范围;
    - --batch_size:   一次处理多少个 PDB 文件 (仅影响进度条刷新频率)。

输出 (graph_mode 两种模式下结构一致, 只是节点数不同):
...
调用格式示例:
    ...
    # 全局节点 (全蛋白) + 仍保留 pocket_mask
    python src/build_graphs_global.py \
        --pdb_dir data/pdb/tpp_family \
        --esm_dir results/sequence_features \
        --out_graph_dir data/graphs_global \
        --sub_resname l02 \
        --tpp_resname l01 \
        --core_cutoff 4.5 \
        --contact_cutoff 5.0 \
...
def build_graph_for_uid(uid: str,
    ...
    参数:
        graph_mode:
            - "pocket": 仅在 pocket 残基处建蛋白节点 (原始行为);
            - "global": 对所有蛋白残基建节点, 但仍用 pocket_mask 标记 core/shell。
    """
    ...
    # 为后续给全局节点打 pocket_mask / pocket_layer 建立映射
    pocket_key_to_layer = {
    ...
    for res_entry in protein_nodes:
        ...
        # pocket mask / layer: 可能是 pocket(core/shell), 也可能是非 pocket
        key = (res_entry["chain_id"], res_entry["resseq"], res_entry["icode"])
        layer = pocket_key_to_layer.get(key, None)
        if layer is None:
    ...
    # 5.1 蛋白-蛋白边 (graph_mode 两种都使用 prot_node_idx, 只是节点集合不同)
...
def main():
    ap = argparse.ArgumentParser(
        description="从 WT holo PDB 生成口袋/全局异质图 (含 ESM 节点特征)"
    )
    ...
    ap.add_argument("--graph_mode", type=str, default="pocket",
                    ...
                    help="pocket: 仅口袋蛋白节点; global: 全蛋白残基节点 (仍保留 pocket_mask)")

    ap.add_argument("--batch_size", type=int, default=1,
                    help="一次处理的 PDB 数目 (仅影响循环粒度和进度条刷新, 不改变结果)")
    ...
    total = len(pdb_files)
    # 动态进度条, 只显示一条, 每处理完一个 PDB 更新一次
    with tqdm(total=total, desc="Building graphs", dynamic_ncols=True) as pbar:
        for start in range(0, total, args.batch_size):
            batch_files = pdb_files[start:start + args.batch_size]
            ...
            for fn in batch_files:
                ...
                try:
                    ...
                    print(f"[WARN] {uid}: failed to parse PDB ({e}), skip.")
                    pbar.update(1)
                    continue
                ...
                try:
                    ...
                    print(f"[WARN] {uid}: failed to load ESM pt ({e}), skip.")
                    pbar.update(1)
                    continue
