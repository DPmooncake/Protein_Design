...
作用:
    使用口袋图 .pt 文件训练 MutPred_v2 自监督模型, 任务为 masked amino-acid 预测。
    模型输入为:
...
调用格式示例:
    python src/train_mutpred_v2.py \
        --graph-dir results/test_graphs \
        --save-dir results/mutpred_v2_ckpt/pocket_test \
        --epochs 100 \
        --batch-size 32 \
        --mask-ratio 0.15 \
        --lr 1e-3 \
...
def apply_random_mask(
    ...
    """
    在 pocket 蛋白节点上随机掩码一部分, 并返回仅在被掩码位置保留的标签。
...
def train_one_epoch(
    ...
    for graph in tqdm(loader, desc="Train", leave=False):
        ...
        labels = build_labels_from_x(x, node_type, pocket_mask)
...
def main():
    ...
    parser.add_argument("--graph-dir", type=str, required=True, help="口袋图 .pt 文件目录")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss = train_one_epoch(
            model, train_loader, optimizer, device, mask_ratio=args.mask_ratio
        )
